...
| 2 | 文档覆盖偏斜 | 文档名碰撞 + 排序偏置 + 无上限 | 唯一 ID + shuffle + max_per_doc |
| 3 | 内容重复文档双倍出题 | 无内容重叠检测 | 内容重叠检测 + 去重池 |

### Round-robin 分析

**引入目的**：将 150 个问题类型标签均匀铺到文档上，保证每个文档拿到多样化类型，避免某文档全是 single_fact。
...
**修法**：保留 round-robin 的均匀分散逻辑，修好两个前提 + 加安全上限。

---

## Step 1: 修好 round-robin 的前提（问题 2 & 3 的根本修复）

### 1.1 修复文档名碰撞 — 用 source_path 作唯一 ID
...
```python
import random
random.shuffle(primary_docs)  # 打乱文档顺序，使年报和研报混合
```

### 1.3 加安全上限 — max_per_doc 参数

当前一个文档可拿 9 个类型（因为 name 碰撞累积），修好碰撞后自然缓解，
但仍加 `max_per_doc` 上限作为安全网：
...
def distribute_across_documents(
    ...
    保留 round-robin 均匀分散逻辑，增加 max_per_doc 上限。
    当某文档已满，跳过它继续分配给下一个文档。
    """
```

效果：150 题 ÷ max_per_doc=3 = 至少覆盖 50 个不同文档。
...
算法：
1. 对所有文档对，取较短文本的采样片段（前 500 字、中间 500 字、末尾 500 字）
2. 检查这些片段是否出现在较长文本中（去空白后子串匹配）
3. 命中率 > threshold → 标记较短文档为 supplementary

**为什么用采样而非全文比较**：全文比较 O(n²) 太慢。采样 3 段 × 500 字 ≈ 1500 字的子串查找，足够判断包含关系。
...
random.shuffle(primary_docs)  # 打乱顺序，打破排序偏置
...
4. 对每对 (excerpt_num_亿, answer_num_亿)：
...
1. **文档分布**：按 source_file 统计题目数，标记占比 >15% 的文档
2. **数值校验**：对所有含数字的题目运行 `validate_answer_numerical_accuracy()`
3. **内容重复**：检测不同 source_file 但 key_entities 高度重叠的题目对
4. **单文档过度集中**：标记同一 source_file 同类型 >2 题的情况
...
- 测试 `doc_id` 使用 source_path，同名文件（不同路径）生成不同 doc_id
...
- 测试 `distribute_across_documents()` 每文档不超过 max_per_doc
- 测试 max_per_doc 上限生效时，多余类型分配给其他文档
- 测试文档数少于类型数时的行为
...
- **不扔掉 round-robin**：保留其均匀分散逻辑，只修好它依赖的前提
