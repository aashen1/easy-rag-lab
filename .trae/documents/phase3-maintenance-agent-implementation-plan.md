...
> 日期：2026-05-05
> 目标：将 checklist 完成率从 87% 提升到 100%

***

## 总览

Phase 3 共 8 个 Task，按依赖关系分为 4 个批次实施：

| 批次       | Task                               | 依赖    | 预计文件变更      |
| -------- | ---------------------------------- | ----- | ----------- |
| B1 基础设施  | T1 SqliteSaver, T2 报告生成, T3 CLI 增强 | 无     | 6 新建 + 3 修改 |
| B2 状态与逻辑 | T4 工具链锁定, T5 模式切换                  | T1    | 3 修改 + 1 新建 |
| B3 UI    | T6 Streamlit 页面                    | T1-T5 | 2 新建 + 2 修改 |
...
用户可通过 `--full` 参数或 UI 切换到全量模式，调用 Meal 批量体系。

### 具体步骤

1. **修改** **`src/agent/state.py`**

   * 新增字段：`full_mode: bool = False`

2. **修改** **`src/agent/prompt.py`**

   * 当 `full_mode=True` 时，提示词中添加"当前为全量模式，可调用 Meal 批量体系处理完整数据集"

   * 当 `full_mode=False` 时，提示词中添加"当前为轻量模式，仅处理 1-2 个 PDF"

3. **修改** **`src/agent/config.py`**

   * 新增 `get_agent_default("full_mode", False)` 配置项

4. **修改** **`config.yaml`**

   * 在 `agent.defaults` 新增 `full_mode: false`

5. **修改** **`src/agent/cli.py`**

   * 新增 `--full` 参数，设置 `full_mode=True`
...
* [ ] `MaintenanceState` 新增 `full_mode` 字段

* [ ] 提示词根据模式动态调整

* [ ] CLI `--full` 参数已实现

* [ ] config.yaml 新增 `full_mode` 配置
