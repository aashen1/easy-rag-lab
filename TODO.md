...
- [ ] 实验报告`baseline.json`当中记录AI回答的`"sources"`字段，给出的来源都是大的总文件名，这策略没啥用感觉，看看能不能细化到标题头或者chunk啥的。这个不着急，跟问题生成策略一块重写（似乎这功能是已经做了。但是因为问题生成策略还是整个MD文档这么来的，所以好像目前会标记一连串十几个doc，标了也白标，命中率还是虚高）
...
- [ ] `exp_configs`当中一些模板yaml是不是有可能已经不符合现在版本了？等实验系统新版做好可以整个整理一下。（已经安排过清洗。不过这个工作随着版本演进还要做的，考虑沉淀为skill或者写入系统提示词）
- [ ] `pixi run pytest tests/ -m "integration" -v` 需要`10 passed, 385 deselected, 1 warning in 182.32s (0:03:02)`，用的时间有点久啊，是不是测试策略不对？看看是否有优化空间 （这好早以前的数据了，等会再测一下换成新的数据）
...
- [ ] CI/CD有空学习下如何做集成
- [ ] pre-commit钩子
- [ ] 考虑是否更换开源许可证，就当是了解一下 Apache 2.0 这个更麻烦的许可证，如何自动化地标记所有源文件
...
- [ ] 断点续传行不行？一个实验涉及100个问题，问到第55个把进程停了，下次回来能不能接着问？（当然了，此种情况要考虑到基模已经变化了，需要在日志中体现出来，就是哪些问题是在什么时候问的，报一个warning，就是这次的结果可靠性降低）（目前好像是非流式的，在整件事干完之前并不会落盘）
- [ ] 慢慢扩大问题集，观察各个指标是否有收敛的趋势，测到收敛就停下来（这个还不着急，得等v0.1.9发版，确认现在测的数据是有用的才行，现在这个虚高逻辑测了也白测）
- [ ] 入库关于“元数据增加基于PDF页码和MD标题层级，改善chunk命中逻辑计算”的对话记录并安排实装
- [ ] 支持部分评测，比如只测PDF→MD这一环节，就不停地换各种文本提取策略，结果同样调API来分析，给出定性判定，就是各种策略出来的MD文件对比如何
- [ ] “Hit Rate”指标进一步扩充到Recall@3/5/10（这个需要先澄清目前有哪些指标。主要考虑两线，RAGAS线那边就是五个指标，builtin线这边让AI摸一下看哪些是做好了的）
...
- [ ] 这评测系统还是没做好，稍微一读就觉得不够solid，还是要尽快地仔细扣一下细节，哪怕从一个PDF和一个问题开始精调呢？（已经安排为目前.8→.9版本的主要工作）
- [ ] 好像现在的commit-rule与CLAUDE.md确实都太长了，去了解一下怎么做渐进式披露更好。开始试着分离一些mcp、skill和rule，精简默认上下文。这个事也可以让AI自己做，用GLM-5.1；
