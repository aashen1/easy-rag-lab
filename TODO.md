...
3. 跑基线测试
4. 检验“花头”的效果
5. 确认本RAG系统目前已有的功能可实现的指标得分上下限如何
...
- [ ] 有时候实验跑完才发现没有开大模型总结功能，又想看AI的报告，这时候是否可以调整一下保存逻辑，做一个“补做LLM报告”的功能？
- [ ] golden test是不是还基于老策略？是否需要重做一下？
- [ ] 断点续传行不行？一个实验涉及100个问题，问到第55个把进程停了，下次回来能不能接着问？（当然了，此种情况要考虑到基模已经变化了，需要在日志中体现出来，就是哪些问题是在什么时候问的，报一个warning，就是这次的结果可靠性降低）
- [ ] 慢慢扩大问题集，观察各个指标是否有收敛的趋势，测到收敛就停下来
- [ ] 支持部分评测，比如只测PDF→MD这一环节，就不停地换各种文本提取策略，结果同样调API来分析，给出定性判定，就是各种策略出来的MD文件对比如何
- [ ] “Hit Rate”指标进一步扩充到Recall@3/5/10
- [ ] LLM报告的“3.1 假设性问题类型表现”这块的提示词还是旧策略的分析方法，看是否要更新下
- [ ] 要求输出完整的透明版报告，预置的问题与答案、问题做emb、recall向量、解码回chunk信息、看到给LLM的提示词、看到LLM的回复、看到各个指标的计算过程……目的：确认基线性能是否虚高、是否存在自问自答虚高、评测指标计算是否有问题
- [ ] 更细粒度的实验记录：如token per chunk的平均值，各个文档的分布，meal整体的分布，最大最小，中位数和方差……
- [ ] 这评测系统还是没做好，稍微一读就觉得不够solid，还是要尽快地仔细扣一下细节，哪怕从一个PDF和一个问题开始精调呢？
