...
### 10.4 2026-04-28：Meal 文件列表显示公司名标签

**需求**：部分年报文件名不含公司名（如 `2023年年度报告.pdf`），公司信息仅在父目录名中（如 `云南白药/2023年年度报告.pdf`），导致用户无法从文件列表识别文档归属。

**技术方案**：

1. **`_extract_company_name(rel_path)`**：从 `MealFile.path`（相对路径）中智能提取公司名。逻辑：
   - 路径层级 < 3 → 无公司目录，返回 `None`
   - 顶层目录不在已知分类（`annual_reports`、`research_reports`）中 → 返回 `None`
   - 文件父目录是年份（4 位数字）→ 向上再取一级（如 `annual_reports/2023/云南白药/xxx.pdf` 取 `云南白药`）
   - 文件父目录是顶层分类 → 返回 `None`（如 `research_reports/xxx.pdf`）
   - 其余情况返回父目录名

2. **去重显示**：仅当公司名**不在文件名中**时才显示标签，避免冗余（如 `恒瑞医药2023年年度报告.pdf` 不重复显示 `🏢 恒瑞医药`）。

3. **UI 样式**：使用 `st.markdown(unsafe_allow_html=True)` 渲染蓝色圆角标签 `<span>`，视觉上与文件名区分。

**显示效果**：

```
2023年年度报告.pdf  🏢云南白药  (4.8 MB)     ← 补充公司名
恒瑞医药2023年年度报告.pdf  (5.7 MB)          ← 文件名已有，不重复
煤炭行业周报：动力煤需求淡季...pdf  (1.9 MB)   ← 研报无公司目录，不显示
```
...
| 函数 | 作用 |
|------|------|
| `_extract_company_name()` | 从相对路径中智能提取公司名 |

**变更函数**：

| 函数 | 变更 |
|------|------|
| `_render_meal_files()` | 有公司名标签时用 `st.markdown` 渲染，否则保持 `st.text` |
