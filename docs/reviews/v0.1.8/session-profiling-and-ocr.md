...
| 实验 | 耗时 | 实际价值 |
|------|------|---------|
| exp_20260425_025347 (OCR on, 第1次) | ~526s | 有效（首次基线，从零解析） |
| exp_20260425_033126 (OCR on, 第2次) | 113.25s | 重复（全缓存命中，仅跑 QA） |
| exp_20260425_034425 (OCR on, 第3次) | 526.46s | 重复（profiling 修复后重跑基线） |
| exp_20260425_043733 (OCR off, hash bug) | 437.38s | 无效（缓存命中假象，S1=152s 实为分块+索引时间） |
| exp_20260425_061926 (OCR on, hash 修复后) | 480.39s | 有效但无新信息（OCR 未执行，与 OCR off 行为一致） |
| exp_20260425_063020 (OCR off, hash 修复后) | 589.11s | 有效但无新信息（S7 LLM 延迟波动拉高总耗时） |

**总计约 2672s（~45 分钟）的 GPU/CPU 时间**，其中只有第一次基线有实际价值。如果一开始就排查两个 Bug，整个 OCR 对比实验可以立即结案，无需跑任何对比。
