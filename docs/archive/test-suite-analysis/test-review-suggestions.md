...
>
> 最后更新：2026-04-18（test-review-fixup spec 执行后）
...
| 2 | ✅ 已修复 | 🟡 中 | 测试质量 | 避免访问 TokenTracker 私有属性 `_records` | test_generator.py |
| 3 | ✅ 已修复 | 🟡 中 | 警告清理 | 修复 PytestCollectionWarning | eval/experiment_reporter.py |
| 4 | ✅ 已修复 | 🟢 低 | 覆盖缺口 | 添加 MRR 对 retrieved_sources 重复项的测试 | test_metrics.py |
| 5 | ✅ 已修复 | 🟢 低 | 覆盖缺口 | 添加 `get_collection_info` 异常返回 None 的测试 | test_indexer.py |
| 6 | ✅ 已修复 | 🟢 低 | 覆盖缺口 | 添加 `build_index` JSONL 加载失败优雅降级的测试 | test_indexer.py |
| 7 | ✅ 已修复 | 🟢 低 | 覆盖缺口 | 添加 `get_llm_config` API key 缺失抛异常的测试 | test_utils.py |
| 8 | ✅ 已修复 | 🟢 低 | 覆盖缺口 | 添加 `retrieve` 返回值 payload 默认值的测试 | test_retriever.py |
| 9 | ❌ 已弃用 | 🟢 低 | 代码优化 | 提取 Pipeline 测试的 7 层 @patch 为辅助函数 | test_pipeline.py |
| 10 | ✅ 已修复 | 🟢 低 | 确定性 | mock_embedder fixture 使用固定向量替代随机向量 | conftest.py |
| 11 | ❌ 已弃用 | 🟢 低 | 规范一致性 | 为旧测试补齐 `@pytest.mark.unit` marker | 多个文件 |
...
### 建议 2：✅ 已修复 — 避免访问 TokenTracker 私有属性
...
**修复方式：** 使用公共 API `tracker.get_records_by_category("rag_qa")` 替代 `tracker._records[0]`，并增加 `len(records) == 1` 断言确保返回结果数量正确。
...
### 建议 4：✅ 已修复 — 添加 MRR 对 retrieved_sources 重复项的测试
...
### 建议 5：✅ 已修复 — 添加 `get_collection_info` 异常路径测试
...
### 建议 6：✅ 已修复 — 添加 `build_index` JSONL 加载失败优雅降级测试
...
### 建议 7：✅ 已修复 — 添加 `get_llm_config` API key 缺失测试
...
### 建议 8：✅ 已修复 — 添加 `retrieve` 返回值 payload 默认值测试
...
### 建议 9：❌ 已弃用 — 提取 Pipeline 测试的 @patch 辅助函数
...
**弃用理由：** 过度修复。当前方式虽视觉上较重，但每个测试的 mock 依赖一目了然，调试时无需跳转到 fixture 定义去理解 mock 行为。提取为 fixture 反而增加间接性、降低可调试性。这不属于 bug 或测试质量问题，纯属风格偏好，不值得为此引入额外复杂度。
...
### 建议 11：❌ 已弃用 — 为旧测试补齐 `@pytest.mark.unit` marker
...
**弃用理由：** 过度修复。补 marker 是纯机械操作，不改善测试质量、不修复任何 bug，且涉及多个文件的批量修改，投入产出比极低。原文档自身也建议"逐步补齐，无需专门安排"，适合在日常开发中顺手补齐。
