...
| ID | 阶段 | 问题 | 实际证据 | 状态 |
|----|------|------|----------|------|
| P6-3 | 评估 | **BuiltinEvaluator contexts/sources 混淆** | faithfulness 收到文件路径而非文本内容，avg_faithfulness=0.628 完全不可信 | ✅ 已修复 |

### 高优先级（直接影响评测结果准确性）

| ID | 阶段 | 问题 | 实际证据 | 状态 |
|----|------|------|----------|------|
| P6-4 | 检索 | **检索结果无文档级去重** | 8/10 问题的 top 5 全部来自同一文档 | 📋 已入库 [FEAT-021](backlog.md) |
| P6-5 | 测试集 | **expected_sources 标注错误** | q010 问光模块公司但 expected 指向中芯国际年报 | 📋 已入库 [BUG-017](backlog.md) |
| P6-2 | 评估 | **检索指标基于文档级** | 同文档不相关 chunk 也算命中，hit_rate=0.875 虚高 | 已由 fix-evaluation-granularity spec 覆盖（chunk 级指标） |
| P2-1 | 分块 | **无重叠导致边界信息丢失** | overlap=0，跨边界信息完全断裂 | 📋 已入库 [OPT-005](backlog.md)（基线刻意为零，待评测链路修复后对比实验） |
| P2-2 | 分块 | **token 级切割不考虑语义边界** | 可能在句子/段落中间硬切 | 语义分块已实现（FEAT-005），基线使用固定分块为设计选择 |
| P3-1 | 向量化 | **512 token 截断冲突** | tiktoken 512 token 可能超过 BGE 512 token 限制 | 📋 已入库 [INV-012](backlog.md) |
| P5-1 | 生成 | **System Prompt 硬编码** | 无法通过实验配置调整 | ✅ 已修复 |
| P5-3 | 生成 | **无 Context 长度控制** | 可能超出模型 context window | 📋 已入库 [FEAT-019](backlog.md) |
| P6-1 | 评估 | **normalize_source 过于宽松** | 仅比较文件名 stem | 📋 已入库 [RF-012](backlog.md) |

### 中优先级（影响效果但非致命）

| ID | 阶段 | 问题 | 影响 | 状态 |
|----|------|------|------|------|
| P1-1 | 解析 | 表格解析质量差 | 财务数据丢失/错乱 | 📋 已入库 [INV-013](backlog.md) |
| P1-3 | 解析 | 页眉页脚污染 | 噪声进入检索 | 📋 已入库 [FEAT-020](backlog.md) |
| P2-3 | 分块 | chunk_id 命名脆弱 | 解析可能出错 | 📋 已入库 [RF-011](backlog.md) |
| P2-4 | 分块 | 无段落感知 | 标题与正文分离 | 部分覆盖于 [INV-010](backlog.md)（元数据增强） |
| P2-5 | 分块 | 长表格被切碎 | 表格数据不完整 | 与 P2-4 相关，待 INV-010 推进 |
| P4-1 | 检索 | 查询未加 BGE 指令前缀 | 检索效果未达最优 | ✅ 已修复 |
| P4-2 | 检索 | 无分数过滤 | 低质量匹配也返回 | ✅ 已修复 |
| P5-2 | 生成 | Context 拼接无排序提示 | LLM 可能被低质量 context 误导 | 📋 待入库 |
| P5-4 | 生成 | 引用来源要求模糊 | LLM 无法真正引用来源 | ✅ 已修复 |
| P6-6 | 评估 | Faithfulness 自评偏差 | 评估不够客观 | 📋 待入库 |
| P6-7 | 评估 | Answer Relevancy 分数不稳定 | 评估结果波动 | 📋 待入库 |
| P6-8 | 评估 | expected_chunks 定位粗糙 | chunk-level 指标虚高 | 📋 待入库 |
