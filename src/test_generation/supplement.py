...
def generate_document_based_questions(
    ...
    type_counts = _calculate_question_distribution(num_questions, type_distribution)
    logger.info(f"Question type distribution: {type_counts}")

    doc_question_plans = _distribute_questions_across_docs(
    ...
    _save_test_set(config, meal_name, test_set, name)
...
def supplement_document_based_questions(
    ...
    _save_test_set(config, meal_name, existing_test_set, test_set_name)
