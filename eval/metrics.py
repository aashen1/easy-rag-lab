...
class QuestionValidity:
    """Question validity assessment result.

    Attributes:
        is_valid: Whether the question is valid for evaluation.
        reason: Reason for validity/invalidity.
        expected_sources_in_corpus: Whether expected sources exist in corpus.
        answerable: Whether the question can be answered from the corpus.
    """
    is_valid: bool
    reason: str
...
def validate_question(
    ...
    """Validate if a test question is valid for evaluation.

    A question is considered valid if its expected_sources exist in the corpus.
    This prevents unfair penalization of retrieval metrics when the expected
    documents are not in the retrieval index.

    Args:
        question: Test question dictionary containing 'source_files' or
            'expected_sources' field.
        corpus_sources: Set of normalized source names in the retrieval corpus.

    Returns:
        QuestionValidity object with validation results.
    ...
    if not sources_in_corpus:
        missing = normalized_expected - normalized_corpus
        return QuestionValidity(
            is_valid=False,
            reason=f"expected sources not in corpus: {missing}",
            expected_sources_in_corpus=False,
            answerable=False,
        )
...
def filter_valid_questions(
    ...
    """Filter out invalid questions that cannot be fairly evaluated.
    ...
    Returns:
        List of valid questions with 'validity' field added.
    """
    valid_questions = []
    invalid_count = 0

    for q in questions:
        validity = validate_question(q, corpus_sources)
        q_with_validity = {**q, "validity": validity}
        if validity.is_valid:
            valid_questions.append(q_with_validity)
        else:
            invalid_count += 1
            logger.warning(
                f"Question {q.get('id', 'unknown')} is invalid: {validity.reason}"
            )

    if invalid_count > 0:
        logger.warning(
            f"Filtered out {invalid_count}/{len(questions)} invalid questions"
        )
